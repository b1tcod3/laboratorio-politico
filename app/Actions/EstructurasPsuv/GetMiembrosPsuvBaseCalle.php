...
class GetMiembrosPsuvBaseCalle
{
	use AsAction;
  ...
  { 
   ...
   ->leftjoin('calles', 'calles.id', '=', 'estructura_psuvs.calle_id')
   ...
   ->when($filters['comunidad'] ?? null, function ($query, $comunidad) {
    $query->where(function ($query) use ($comunidad) {
      $query->where('comunidads.id',$comunidad);
    });
  })
   ->when($filters['calle'] ?? null, function ($query, $calle) {
   ...
   if($withPagination){
    ...
    ->through(fn ($miembro) => [
      ...
      'comunidad_id' => $miembro->comunidad_id,
      'calle_id' => $miembro->calle_id,
      'comunidad_nombre' => $miembro->comunidad_nombre,
      'calle_nombre' => $miembro->calle_nombre,
