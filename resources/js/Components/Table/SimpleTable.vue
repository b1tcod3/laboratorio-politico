0<script setup>
    ...
    onMounted( () => {
        if (document.getElementById(props.name) && typeof simpleDatatables.DataTable !== 'undefined') {
        ...
        .fill("+")
            ...
            link.download = (options.filename || props.name) + ".txt";
            // Append the link
...
<template>
	<div class="p-4 w-full h-full">
        ...
        <table :id="name">
            ...
            <tbody>
                <tr class="hover:bg-gray-50 dark:hover:bg-gray-800 cursor-pointer" v-for="d in data">
                        <td class="font-medium text-gray-900 whitespace-nowrap dark:text-white">{{d['id']}}</td>

                        <template v-for="(field,id) in fields">
                            <td  v-if="types_fields[id]=='string'">{{d[field]}}</td>
                            <td v-if="types_fields[id]=='image'">
                            <img class="w-10 h-10 rounded-full" :src="d[field]" alt="image">
                            </td>
                        </template>
                        <td class="px-4 py-3 flex items-center justify-end">
                                
                                            <Link :href="`${route}/${d['id']}`" class="block py-2 px-4 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white">Show</Link>
                                  
                                            <Link :href="`${route}/${d['id']}/edit`" class="block py-2 px-4 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white">Edit</Link>
