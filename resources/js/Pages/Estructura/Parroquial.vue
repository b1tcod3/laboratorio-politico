...
const closeModal = () => {
  ...
  const fields = [{'id':1,'name_display':'ID','name':'id'},
        {'id':2,'name_display':'Municipio','name':'municipio_parroquia_nombre'},
        {'id':3,'name_display':'Parroquia','name':'parroquia_nombre'},
        {'id':4,'name_display':'Cédula','name':'cedula'},
        {'id':5,'name_display':'Cargo','name':'cargo_nombre'},
        {'id':6,'name_display':'Nombre','name':'nombres'},
        {'id':7,'name_display':'Apellido','name':'apellidos'},
        {'id':8,'name_display':'Sexo','name':'sexo'},
        {'id':9,'name_display':'Teléfono','name':'telefono_movil'},
  ...
  const form = useForm({
          ...
          municipio_parroquia: props.filters.municipio_parroquia,
  ...
  const updateTable = (column) => {
      loading.value = true;
      form.get('/estructuras/EPP', pickBy(form), { preserveState: true,
          onFinish: () => {
            loading.value=false;
          }
        })
...
<template>
  <div>
    <Head title="EPP" />
    <h1 class="mb-8 text-3xl font-bold">Estructura Política Parroquial</h1>
...
<div class="grid gap-6 mb-6 md:grid-cols-3">
  ...
  <SelectForm label="Municipio" v-model="form.municipio_parroquia" :options="municipios" nameOption="nombre"
            :error="form.errors.municipio_parroquia" @change="form.parroquia='';updateTable()"
            />
  <SelectForm label="Parroquia" v-model="form.parroquia" :options="parroquias[form.municipio_parroquia]" nameOption="parroquia_nombre"
            :error="form.errors.parroquia" @change="updateTable"
            :disabled="form.municipio_parroquia==''"
            />
  <div >
...
<Modal :show="showModal" @close="closeModal" >
       <EstructuraPsuvForm @closeModal="closeModal" :miembro="miembro" :cargos="cargos" :municipios="municipios" :municipio="form.municipio_parroquia" :parroquias="parroquias" :parroquia="form.parroquia"/>
</Modal>
