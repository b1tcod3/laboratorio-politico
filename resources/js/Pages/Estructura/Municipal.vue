...
const closeModal = () => {
  ...
  const fields = [{'id':1,'name_display':'ID','name':'id'},
        ...
        {'id':4,'name_display':'Cargo','name':'cargo_nombre'},
  ...
  const updateTable = (column) => {
      loading.value = true;
      form.get('/estructuras/EPM', pickBy(form), { preserveState: true,
          onFinish: () => {
            loading.value=false;
          }
        })
...
<template>
  <div>
    <Head title="EPM" />
    <h1 class="mb-8 text-3xl font-bold">Estructura Política Municipal</h1>
...
<div class="grid gap-6 mb-6 md:grid-cols-3">
  ...
  <SelectForm label="Municipio" v-model="form.municipio" :options="municipios" nameOption="nombre"
            :error="form.errors.municipio" @change="updateTable"
            />
  <div >
...
<Modal :show="showModal" @close="closeModal" >
       <EstructuraPsuvForm @closeModal="closeModal" :miembro="miembro" :cargos="cargos" :municipios="municipios" :municipio="form.municipio"/>
</Modal>
