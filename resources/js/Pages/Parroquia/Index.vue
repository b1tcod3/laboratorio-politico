<script setup>
  ...
  const updateTable = () => {
      loading.value = true;
      form.get(route('parroquias.index'), pickBy(form), { preserveState: true,
          onFinish: () => {
            loading.value=false;
          }
        })
...
<template>
  ...
  <container-table name="Parroquias" :links="parroquias" :isEmpty="parroquias.data.length===0">
  <template #widget>
    ...
    <div class="grid gap-4 sm:grid-cols-3 sm:gap-6 mx-4">
      ...
      >
        ...
        <option :value="municipio.nombre" v-for="municipio in getMunicipios">{{municipio.nombre}}</option>
      </select>  
      ...
      <div>
        ...
        <div class="relative">
            ...
            <input type="text" id="table-search" class="block p-2 ps-10 text-sm text-gray-900 border border-gray-300 rounded-lg w-80 bg-gray-50 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="Filtrar parroquia" v-model="form.search">
        </div>
  </template>

  <template #columns>
    <th scope="col" class="px-6 py-3"
    v-for="column in columnsTable.columns_parroquia"
    >
  ...
  <template #body>
    <tbody v-show="!loading">
            ...
            <tr v-for="parroquia in parroquias.data" class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                <td class="px-6 py-4">
                    {{parroquia.id}}
                </td>
                <td class="px-6 py-4">
                ...
                <td class="px-6 py-4">
                    <Link :href="route('parroquias.show',{parroquia:parroquia})" type="button" class="px-3 py-2 text-xs font-medium text-center text-white bg-purple-700 rounded-lg hover:bg-purple-800 focus:ring-4 focus:outline-none focus:ring-purple-300 dark:bg-purple-600 dark:hover:bg-purple-700 dark:focus:ring-purple-800">Ver</Link>
