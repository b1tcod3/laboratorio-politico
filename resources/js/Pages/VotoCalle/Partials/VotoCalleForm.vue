...
const fetchData = () => {
  ...
  .then(data => {
    ...
    if(data.status=='ok')
    {
        ...
        if(props.miembro.municipio_parroquia_id){
            form.municipio = props.miembro.municipio_parroquia_id;
        }
...
<template>
    <div class="p-6">
        ...
        <div class="grid gap-6 mb-6 md:grid-cols-3" v-if="infoMiembro">
            ...
            <SelectForm v-if="parroquias" label="Municipio" v-model="form.parroquia" :options="parroquias[form.municipio]" nameOption="parroquia_nombre"
