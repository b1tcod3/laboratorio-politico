<script setup>
  ...
  import CalleForm from './Partials/CalleForm.vue';
...
const closeModal = () => {
  ...
  const fields = [{'id':1,'name_display':'ID','name':'id'},
        ...
        {'id':7,'name_display':'CALLE','name':'calle_nombre'},
        {'id':8,'name_display':'Miembros Psuv','name':'sum_miembros_calles'},
  ...
  const form = useForm({
          ...
          comunidad: props.filters.comunidad?props.filters.comunidad:"",
  ...
  const updateTable = (column) => {
      loading.value = true;
      form.get('/calles', pickBy(form), { preserveState: true,
          onFinish: () => {
            loading.value=false;
          }
        })
  ...
  buttonsStyling: true
    ...
    swalDelete.fire({
  title: '¿Confirmar Eliminar Calle? '+calle.calle_nombre,
  ...
  if (result.isConfirmed) {
    swalDelete.fire(
      'Excelente!',
      'Calle Eliminada con éxito',
      'success'
    );
    router.delete(route('calles.destroy',calle.id),{
      onSuccess: () => console.log('borrado:'+calle.calle_nombre),
    });
...
<template>
  <div>
    <Head title="Calle" />
    <h1 class="mb-8 text-3xl font-bold">Calles</h1>
...
<div class="grid gap-6 mb-6 md:grid-cols-3">
  ...
  <SelectForm label="Comunidad" v-model="form.comunidad" :options="comunidades[form.centro_electoral]" nameOption="comunidad_nombre"
            :error="form.errors.comunidad" @change="form.comunidad=updateTable()"
  ...
  <div class="relative overflow-x-auto shadow-md sm:rounded-lg">
    ...
    <tbody v-show="!loading">
      ...
      v-for="calle in calles.data" :key='calle.id'
      >
...
<pagination :links="calles.links" :total="calles.total" :from="calles.from" :to="calles.to"/>
</div>

<Modal :show="showModal" @close="closeModal" >
       <CalleForm @closeModal="closeModal" :municipios="municipios" :municipio="form.municipio" :parroquias="parroquias" :parroquia="form.parroquia" :centros_electorales="centros_electorales" :centro_electoral="form.centro_electoral" 
       :comunidades="comunidades" :comunidad="form.comunidad" :calle="calle" />
</Modal>
